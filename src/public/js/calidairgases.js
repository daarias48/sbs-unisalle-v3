...
let link='CalidAir/CalidAirPM_GAS48BD9E7C803B/Data'
...
const url = 'https://calidair-d27b9-default-rtdb.firebaseio.com/CalidAir/CalidAirPM_GAS48BD9E7C803B/Data.json?orderBy=%22id%22&limitToLast=60&print=pretty';
fetch(url)
.then(res => res.json())
.then(data => {   
...
function tabla(data){
    ...
    for (var key in data) {
        ...
        no2_s.push(data[key].no2.toFixed(2));
        o3_s.push(data[key].o3.toFixed(2));
        co_s.push(data[key].co.toFixed(2));
     }
     
     //Espacios para última medición
     date.innerHTML = `${day_s[day_s.length-1]}, `
     hour.innerHTML = `${hour_s[hour_s.length-1]}`
     no2.innerHTML = `${no2_s[no2_s.length-1]} µg/m<sup>3</sup>`
...
const downloadExcel = (data) => {
    ...
    saveAsExcel(excelBuffer, 'Mediciones-CalidAirGases')
}
...
onChildAdded(commentsRef, (data) => {
    ...
    no2.innerHTML = `${nuboair.no2.toFixed(2)} ppb`
...
onValue(reference, (snap) => {
    ...
    for (const key in data) {
        ...
        allCO.push(data[key].co)
        allO3.push(data[key].o3)
    }
    allDates.map(el => {
    ...
    const co = allCO.reverse().filter((el, i) => i < 20).reverse()
    const o3 = allO3.reverse().filter((el, i) => i < 20).reverse()
    ...
    function updateSelect() {
        ...
        switch(measureModulair) {
            ...
            case "2": 
                ...
                myChart.data.labels = hour
                myChart.data.datasets[0].data = co
                myChart.data.datasets[0].borderColor="#56083E"
                myChart.data.datasets[0].backgroundColor="#C72C97"
                myChart.data.datasets[0].label = `CO ppb`
                myChart.update()
            ...
            case "4": 
                myChart.data.labels = hour
                myChart.data.datasets[0].data = o3
                myChart.data.datasets[0].borderColor="#234783"
                myChart.data.datasets[0].backgroundColor="#13284B"
                myChart.data.datasets[0].label = `O3 ppb`
                myChart.update()
